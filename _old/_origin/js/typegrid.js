...
// 		for(var cnt = 0; cnt < floorNum; cnt ++){
	...
	//create
	function createAllWrap() {
		// typegrid.jsで生成される全ての要素をまとめる親要素を生成する関数
	...
	function getViewBox(){
		...
		viewBoxValue.push(getWidth());
		viewBoxValue.push(" ");
		viewBoxValue.push(getHeight());
		var svgViewBox = viewBoxValue.join('');
		return svgViewBox;
	}
	//set
	function resetSvgView(targetSvg) {
		targetSvg.textContent = null;
		targetSvg.removeAttribute("viewBox");
		targetSvg.setAttribute("viewBox", getViewBox());
		targetSvg.setAttribute("width", getWidth());
		targetSvg.setAttribute("height", getHeight());
	}
	...
	function setGrids() {
		...
		var colBody = getId(tgPrefix + "colBody");
			resetSvgView(gridBody);
			resetSvgView(colBody);
...
// 			colBody.appendChild(loopColX());
	...
	function redrawGrid(currentFontSize) {
		...
		if(currentSize == undefined){
			...
			console.log("viewboxの値：" + getViewBox());
			console.log('↑result.');
